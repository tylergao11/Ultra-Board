...
1. 沿着题材从出生至日期 T 的运行轨迹，判断资金下一次最容易回到哪里；
2. 在该方向的空中梯队中，判断哪只股票在 T+1 做出什么动作才属于超预期，并且该动作是否有能力重新带动板块发酵。

方向选择与个股选择必须分开：题材资金轨迹决定去哪里找，个股自身动作决定能不能做。竞价只负责验证预案，不能到竞价时才临时发明方向。
...
来源标签只是线索，不是资金因果。需要把承担同一资金任务的产业链合并观察，例如算力—通信—PCB—光模块可以属于同一个 AI 硬件资金池，机器人—工业母机—自动化设备可以属于同一个机器人资金池。

- 保留股票的全部候选属性，不机械服从单一主标签；
- 多属性股票可能同时连接两个资金池，不能因为标签重叠就断言由谁带动；
- 公告重组、控制权变更、复牌、摘帽等事件驱动必须单独标记，不能用其高度冒充自然题材高度；
- 一字板不因“一字”自动排除。它可以证明板块强度，但没有日内交易机会；是否代表自然题材，取决于驱动而不是板型。
...
判断必须相对于题材自己的前态，不能只看 T 日涨停家数。至少回答：

- 最早何时出现有意义的共同扩散，而不是孤立涨停；
- 哪一天建立高、中、低梯队；
- 哪一天扩散过猛，哪些成员在下一日消失、触板失败或继续晋级；
- T 日属于持续主升、内部淘汰、首次回流、二次回流还是尾盘同层脉冲；
- 当前仍在持续吸纳资金，还是已经离开后重新被接受。

“当前主流”与“最容易回流”不是同一个问题：资金一直没有离开的方向叫延续或内部淘汰，不应伪装成回流。
...
5. 排除同步新闻、指数共振、另一只股票先启动和多属性切换等解释。
...
驱动越新、越硬，市场默认预期越高；持续多日且已经大面积发酵的逻辑，即使基本面更强，也更难在 T+1 再形成超预期。板块故事只能解释可能交易的属性，不能直接证明某方向打崩、抽血或带动另一方向。
