...
这里预留逐日研究形成的**原子方法摘要**。它不是第二套行情数据，也不替代历史案例正文。

## 知识职责与真相源

- [`../cases/records`](../cases/records) 只保存由结构化冻结计划和正式行情判卷生成的新案例；当前为空，字段与时间边界见 [`../../docs/历史案例契约.md`](../../docs/历史案例契约.md)。
- `summaries.jsonl` 当前为空，不参与每日判断，也不得成为第二套方法规则。
...
当前 `summaries.jsonl` 为空，不从旧学习材料恢复内容。未来新增摘要必须来自已冻结并完成判卷的新记录。
