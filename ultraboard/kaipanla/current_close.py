...
def _load_ths_codes(day_dir: Path, day: str) -> set[str]:
    ...
    if not path.exists():
        raise RuntimeError(f"缺少同花顺涨停池: {path}")
    ...
    *,
    ...
    for source_position, sector in enumerate(sectors_raw, 1):
        ...
        for stock in parsed:
            ...
            if is_bse(code):
                raise RuntimeError(f"当前目标市场意外出现北交所代码: {code}")
            seen.add(code)
            stocks.append(stock)
    ...
    if len(stocks) != sjzt:
        raise RuntimeError(f"开盘啦市场方向非 ST {len(stocks)} 只，与 SJZT {sjzt} 不一致")
    ...
    source_counts = {
        **{str(pid): int(board_counts.get(str(pid), 0)) for pid in range(1, 5)},
        "5": sum(count for board, count in board_counts.items() if int(board) >= 5),
    }
