...
days = ["2025-01-17", "2025-01-20", "2025-01-21", "2025-01-22", "2025-01-23", "2025-01-24"]
print("==== 兴业 603928 ====")
trail(CODE, days)
...
for d in days:
    ...
    chip_theme = [s for s in stocks if str(s.get("theme") or "") == "芯片"]
    chip_tag = [s for s in stocks if "芯片" in blob(s) or "光刻胶" in blob(s)]
    print()
    print("=" * 8, d, "涨停", data.get("count"), "最高", data.get("max_board"), "=" * 8)
    print("主标签", Counter(str(s.get("theme") or "?") for s in stocks).most_common(8))
    print(f"芯片主 {len(chip_theme)} 芯片/光刻胶标签 {len(chip_tag)}")
    print("-- 芯片相关 --")
    for s in sorted(chip_tag, key=lambda x: (-int(x.get("boards") or 0), ts(x.get("first_limit_ts")))):
    ...
    for s in sorted(stocks, key=lambda x: (-int(x.get("boards") or 0), ts(x.get("first_limit_ts")))):
        is_yz, first, oc, to = yizi(s)
        if is_yz and int(s.get("boards") or 0) >= 2:
            print(
                f"  {s.get('name')} {s.get('boards')}板 主题={s.get('theme')} "
                f"属性={tags_of(s)} 换手={to}"
            )
