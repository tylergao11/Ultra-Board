...
DAYS = ["2024-12-05", "2024-12-06", "2024-12-09", "2024-12-10", "2024-12-11"]
...
for d in DAYS:
    ...
    s = next((x for x in stocks if x["name"] == "黄河旋风"), None)
    if not s:
        print("  黄河旋风 不在池")
    else:
        raw = s.get("raw") or []
        print(
            f"  黄河旋风 {s.get('code')} {s.get('boards')}板 换手{s.get('turnover_rate')} "
...
req = urllib.request.Request(
    "https://q.stock.sohu.com/hisHq?code=cn_600172&start=20241204&end=20241212&stat=1&order=D",
    headers={"User-Agent": "Mozilla/5.0"},
)
try:
    print("\n== sohu ==")
    print(urllib.request.urlopen(req, timeout=15).read().decode("gbk", "replace")[:1500])
