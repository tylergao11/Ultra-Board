...
def robot(s):
    b = str(s.get("theme") or "") + " " + tags_of(s)
    return "机器人" in b
...
# find code
hd = None
for d in ("2025-01-09", "2025-01-03"):
    for s in load(d).get("stocks") or []:
        if s.get("name") == "海得控制":
            hd = str(s.get("code")).zfill(6)
            break
    if hd:
        break
print("code", hd)
trail(hd or "XXXXXX", days)

print("==== 金奥博 ====")
ja = None
for d in ("2025-01-10", "2025-01-09"):
    for s in load(d).get("stocks") or []:
        if s.get("name") == "金奥博":
            ja = str(s.get("code")).zfill(6)
            break
    if ja:
        break
print("code", ja)
trail(ja or "XXXXXX", days)

for d in ("2025-01-08", "2025-01-09", "2025-01-10", "2025-01-13"):
    data = load(d)
    stocks = data.get("stocks") or []
    rob_theme = [s for s in stocks if str(s.get("theme") or "") == "机器人概念"]
    rob_tag = [s for s in stocks if robot(s)]
    print()
    ...
    print(f"机器人主标签 {len(rob_theme)}  主题或标签含机器人 {len(rob_tag)}")
    print("-- 机器人 --")
    for s in sorted(rob_tag, key=lambda x: (-int(x.get("boards") or 0), ts(x.get("first_limit_ts")))):
        show(s)
    print("-- 2板+ --")
