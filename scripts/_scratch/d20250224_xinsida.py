...
WANT = ("杭齿前进", "新时达", "泰豪科技", "大位科技", "庄园牧场", "云赛智联", "新华传媒", "新华网")
...
for d in days:
    ...
    for s in load(d).get("stocks") or []:
        n = s.get("name") or ""
        if n in WANT:
            codes[n] = str(s.get("code")).zfill(6)
        if "环瑞" in n or n.startswith("环"):
            extra.append((d, n, s.get("code"), s.get("boards"), s.get("theme"), tags_of(s)))

print("codes", codes)
print("环* extras sample", extra[:20])

for name in ("杭齿前进", "新时达", "泰豪科技", "大位科技", "庄园牧场", "云赛智联", "新华传媒"):
    print(f"==== {name} {codes.get(name)} ====")
    if codes.get(name):
        trail(codes[name], days)

for d in ("2025-02-21", "2025-02-24"):
    data = load(d)
    ...
    print("-- 点名 --")
    for s in stocks:
        if s.get("name") in WANT or "环瑞" in str(s.get("name") or ""):
            show(s)
    print("-- 2板+ --")
    ...
    print("-- 一字 2板+ --")
    for s in sorted(stocks, key=lambda x: (-int(x.get("boards") or 0), ts(x.get("first_limit_ts")))):
        if yizi(s)[0] and int(s.get("boards") or 0) >= 2:
            show(s)
    rob = [s for s in stocks if "机器人" in (str(s.get("theme") or "") + tags_of(s))]
    print(f"-- 机器人 {len(rob)} --")
    for s in sorted(rob, key=lambda x: (-int(x.get("boards") or 0), ts(x.get("first_limit_ts"))))[:12]:
        show(s)
