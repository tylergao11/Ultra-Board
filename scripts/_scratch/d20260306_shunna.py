...
def shunna(day):
    ...
    if hit:
        return (
            f"在池 {hit.get('boards')} {hit.get('boards_desc') or ''} "
            f"主题={hit.get('theme')} 属性={tags_of(hit)} 换手={hit.get('turnover_rate')} "
            f"首封={ts(hit.get('first_limit_ts'))} 开板={oc_of(hit)}"
        )
    op = THS / f"{day}.json"
    if op.exists():
...
for d in ("2026-03-05", "2026-03-06", "2026-03-09"):
    data = load(d)
    ...
    chem_s = [s for s in data.get("stocks") or [] if chem(s)]
    suan_s = [s for s in data.get("stocks") or [] if suan(s)]
    print("化工", len(chem_s), "算力属性/主题", len(suan_s))
