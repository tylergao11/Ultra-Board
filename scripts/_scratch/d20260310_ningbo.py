...
def show(s):
    ...
    print(
        ...
        f"开={s.get('open')} 高={s.get('high')} 低={s.get('low')} 开幅={s.get('open_pct')}"
    )
    ...
    for d in days:
        ...
        if hit:
            ...
            print(
                ...
                f"换手={to} 首封={first} 开板={oc} "
                f"开={hit.get('open')} 低={hit.get('low')} 开幅={hit.get('open_pct')} "
...
# resolve 宁波建工 code
nb_code = None
for d in ("2026-03-09", "2026-03-10", "2026-03-06"):
    p = ROOT / d / "zt_pool.json"
    if not p.exists():
        continue
    for s in load(d).get("stocks") or []:
        if s.get("name") == "宁波建工":
            nb_code = str(s.get("code")).zfill(6)
            break
    if nb_code:
        break

days = ["2026-03-05", "2026-03-06", "2026-03-09", "2026-03-10"]
print("宁波建工", nb_code)
print("==== 顺钠 000533 ====")
trail("000533", days)
print("==== 宁波建工 ====")
trail(nb_code or "XXXXXX", days)
print("==== 王力安防 ====")
wl = None
for d in days:
    p = ROOT / d / "zt_pool.json"
    if not p.exists():
        continue
    for s in load(d).get("stocks") or []:
        if "王力" in str(s.get("name") or ""):
            wl = str(s.get("code")).zfill(6)
            break
    if wl:
        break
print("code", wl)
...
for d in ("2026-03-06", "2026-03-09", "2026-03-10"):
    p = ROOT / d / "zt_pool.json"
    if not p.exists():
        print(d, "NO FILE")
        continue
    ...
    print("主标签", Counter(str(s.get("theme") or "?") for s in stocks).most_common(14))
    print("-- 2板+ --")
    ...
    for s in sorted(stocks, key=lambda x: (-int(x.get("boards") or 0), ts(x.get("first_limit_ts")))):
        if yizi(s)[0]:
            show(s)
    # 地产/王力旁边
    fang = [s for s in stocks if any(k in (str(s.get("theme") or "") + tags_of(s) + str(s.get("name") or "")) for k in ("地产", "王力", "装修", "物业"))]
    print(f"-- 地产相关 {len(fang)} --")
    for s in fang:
        show(s)
