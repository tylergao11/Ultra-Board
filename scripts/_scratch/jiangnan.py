...
DAYS = ["2024-12-09", "2024-12-10", "2024-12-11", "2024-12-12", "2024-12-13"]
...
for d in DAYS:
    ...
    s = next((x for x in stocks if x["name"] == "江南高纤"), None)
    if not s:
        print("  江南高纤 不在池")
    else:
        raw = s.get("raw") or []
        print(
            f"  江南高纤 {s.get('code')} {s.get('boards')}板 换手{s.get('turnover_rate')} "
    ...
    for key in ["化工", "化纤"]:
        ...
        main = [x for x in stocks if (x.get("theme") or "") == key]
        if not rows:
            continue
...
req = urllib.request.Request(
    f"https://q.stock.sohu.com/hisHq?code=cn_{code}&start=20241208&end=20241213&stat=1&order=D",
