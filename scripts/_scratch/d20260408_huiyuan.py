...
def is_tx(s):
    b = blob(s)
    return any(k in b for k in ("通信", "光纤", "光模块", "CPO", "5G", "卫星"))
...
for d in ("2026-04-01", "2026-04-02", "2026-04-03", "2026-04-07", "2026-04-08"):
    data = load(d)
    ...
    tx_tag = [s for s in stocks if is_tx(s)]
    tech_tag = [s for s in stocks if is_tech(s)]
    print(
        f"医药主 {len(med_theme)} 医药标签 {len(med_tag)} | "
        f"通信主 {len(tx_theme)} 通信标签 {len(tx_tag)} | 科技标签 {len(tech_tag)}"
    )
    print("-- 医药 --")
    ...
    for s in sorted(stocks, key=lambda x: (-int(x.get("boards") or 0), ts(x.get("first_limit_ts")))):
        if not (is_tx(s) or is_tech(s)):
            continue
        if s.get("code") in seen:
            continue
        seen.add(s.get("code"))
        is_yz, first, oc, to = yizi(s)
        print(
