...
for d in ("2025-02-27", "2025-02-28"):
    data = load(d)
    ...
    if n == 0:
        print("  无")
    print("-- 09:25且开板0，换手不论 --")
    n2 = 0
    for s in sorted(stocks, key=lambda x: (-int(x.get("boards") or 0), float(x.get("turnover_rate") or 99))):
        first = ts(s.get("first_limit_ts"))
        oc = oc_of(s)
        to = float(s.get("turnover_rate") or 99)
    ...
    if n2 == 0:
        print("  无")
    zhuang = next((s for s in stocks if s.get("name") == "庄园牧场"), None)
    dawei = next((s for s in stocks if s.get("name") == "大位科技"), None)
    print("庄园", "在" if zhuang else "不在", "大位", "在" if dawei else "不在")
